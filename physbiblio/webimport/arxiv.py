...
class webSearch(webInterf):
	...
	def arxivDaily(self, category):
		...
		if text is None:
			...
			return False
		author = re.compile('>([^<]*)</a>')
		mainCat = re.compile('\[([\-\.a-zA-Z]*)\]')
		versionFinder = re.compile('arXiv:([0-9\.v]*) ')
		...
		try:
			...
			for element in data.entries:
				...
				tmp["authors"] = [ m.group().replace("</a>", "").replace(">", "") for m in author.finditer(element["authors"][0]["name"]) if m != "" ]
