	...
	def test_exportForTexFile(self):
		...
		with patch('physbiblio.database.catsEntries.insert', return_value = True) as _ceins:
			with patch('physbiblio.database.entries.getByBibtex', side_effect = [
					...
					[],
					[],
					[{"bibkey": "newcite", "bibtexDict": {}, "bibtex": '@article{newcite,\nauthor= "myself",\ntitle="{some paper}",\n}'}],
					]) as _getbbibt:
				with patch('physbiblio.database.entries.loadAndInsert',
						side_effect = ['', '@article{newcite,\nauthor= "myself",\ntitle="{some paper}",\n}']) as _mock:
		...
		with patch('physbiblio.database.entries.getByBibtex', side_effect = [
				[{"bibkey": "newcite", "bibtexDict": bibtexparser.loads(bibtex1).entries[0], "bibtex": bibtex1}],
				[{"bibkey": "bib1", "bibtexDict": bibtexparser.loads(bibtex2).entries[0], "bibtex": bibtex2}],
				[{"bibkey": "bib2", "bibtexDict": bibtexparser.loads(bibtex3).entries[0], "bibtex": bibtex3}],
				]) as _getbbibt:
			output = pBExport.exportForTexFile(testTexName, testBibName, autosave = False, updateExisting = True)
		...
		with patch('physbiblio.database.entries.getByBibtex', side_effect = [
			...
			output = pBExport.exportForTexFile(testTexName, testBibName, autosave = False, removeUnused = True)
		...
		with patch('physbiblio.database.entries.getByBibtex', side_effect = [
			...
			output = pBExport.exportForTexFile(testTexName, testBibName, autosave = False, overwrite = True)
		self.assertEqual(output[0], ['newcite']) #requiredBibkeys
		self.assertEqual(output[1], []) #missing
		self.assertEqual(output[2], []) #retrieved
		self.assertEqual(output[3], []) #notFound
